...
interface ReceptionTicketsProps {
  ...
  entregasList: { id: string; cajas: number; peso: number; bono: boolean }[];
  setEntregasList: React.Dispatch<
...
export default function ReceptionTickets({
  ...
  const itemsPerPage = 12; // 6 y 6 en dos columnas
  const totalPages = Math.ceil(entregasList.length / itemsPerPage);
  const paginatedEntregas = entregasList.slice(
      ...
      >
        <div className="space-y-6">
          ...
          <div>
            ...
            <div
              className={`min-h-[350px] ${entregasList.length === 0 ? "space-y-3" : "grid grid-cols-1 md:grid-cols-2 md:grid-rows-6 md:grid-flow-col gap-4 content-start"}`}
            >
